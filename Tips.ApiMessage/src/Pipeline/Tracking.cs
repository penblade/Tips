...
namespace Tips.Pipeline
{
    public static class Tracking
    {
        ...
        {
            var traceParentStateString = TraceParentStateString ?? string.Empty;
            var states = traceParentStateString.Split(',').ToList();
            return states.Count >= 1 ? states.First() : string.Empty;
        }
    }
}
